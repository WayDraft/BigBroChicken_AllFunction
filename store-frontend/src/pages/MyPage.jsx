...
export default function MyPage() {
  ...
  return (
    <div className="w-full py-20">
      <div className="flex flex-col items-center justify-center gap-16 mb-20 shadow-md">
        <span className="text-3xl">마이페이지</span>
        <div className="flex flex-row items-center justify-between w-1/3 text-xl pb-10">
          <button
            onClick={() => setChooseMenu(true)}
            className={`${chooseMenu ? 'text-burgundy underline' : ''}`}
          >
            주문/배송 조회
          </button>

          <button
            onClick={() => setChooseMenu(false)}
            className={`${chooseMenu ? '' : 'text-burgundy underline'}`}
          >
            회원 정보
          </button>
      ...
      {chooseMenu ? <MyOrder /> : <MyInformation />}
    </div>
  )
}
